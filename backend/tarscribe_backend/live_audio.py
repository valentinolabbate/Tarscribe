"""PCM chunk validation and append logic for live recording sessions."""
...
def validate_and_append_chunk(
    ...
    """Validate a PCM16 chunk and append it to the raw PCM file.

    Idempotent for already-confirmed sequences; rejects gaps.
    """
...
def cleanup_session_dir(pcm_path: str | None) -> None:
    """Remove the per-session PCM directory after the session is done.

    Safe to call from a background thread; ignores errors silently.
    """
    if not pcm_path:
        return
    import shutil
...
def build_wav_window(
    ...
    """Extract [start_sec, end_sec) from the raw PCM file and write as WAV.

    Returns False if there is no audio in that range.
    """
